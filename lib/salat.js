...
function sholatAll(kota) {
    return new Promise(async (resolve, reject) => {
        ...
        }); resolve({ creator: 'DannLonely', status: true, data })
    })
}
...
module.exports = (kota) => {
    return new Promise(async (resolve, reject) => {
        ...
        if (!sholat.status) return resolve({ creator: 'Nabildzr', status: false })
        let now = moment(new Date * 1).format('DD-MM-yy')
        let data = sholat.data.find(v => v.tanggal == now)
        if (typeof data == 'undefined') return resolve({ creator: 'Nabildzr', status: false })
        resolve({ creator: 'Nabildzr', status: true, data })
    })
}
